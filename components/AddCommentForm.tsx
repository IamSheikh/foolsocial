...
const AddCommentForm: FC<Props> = (props) => {
  ...
  const handleSubmit = (e: { preventDefault: () => void }) => {
    e.preventDefault()
    props.handler(comment)
  }
  return (
    <div className='mt-2 text-center mt-10'>
      ...
      <form onSubmit={handleSubmit} className='flex justify-center mt-4'>
        <div className='mb-2'>
          ...
          />
          <div>
            <button className='bg-teal-500 mt-2 text-white py-2 px-4 rounded duration-1000 ml-3 hover:bg-white hover:text-teal-500'>Comment</button>
          </div>
        </div>
      </form>
    </div>
